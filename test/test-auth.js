...
// 			return chai.request(app).post('/api/auth/login').send({username: 'wrongUsername', password }).then(() => expect.fail(null, null, 'Request should not succeed')).catch(err => {
// 				if(err instanceof chai.AssertionError) {
// 					throw err;
// 				}
...
// 		it('Should reject requset with no credentials', function () {
// 			return chai.request(app).post('/api/auth/refresh').then(() =>
// 				expect.fail(null, null, 'Request should not succeed')).catch(err => {
// 					if(err instanceof chai.AssertionError){
// 						throw err;
// 					}
...
// 		it('Should reject request with an invalid todken', function() {
...
// 			return chai.request(app).post('/api/auth/refresh').set('authorization', `Bearer ${token}`).then(() => expect.fail(null, null, 'Request should not succeed')).catch(err => {
// 				if (err instanceof chai.AssertionError) {
// 					throw err;
// 				}
...
// 			return chai.request(app).post('/api/auth/refresh').set('authorization', `Bearer ${token}`).then(() => 
// 				expect.fail(null, null, 'Request should not succeed')).catch(err => {
// 					if(err instanceof chai.AssertionError) {
// 						throw err;
// 					}
...
// 			return chai.request(app).post('/api/auth/refresh').set('authorization', `Bearer ${token}`).then(res => {
// 				expect(res).to.have.status(200);
// 				expect(res.body).to.be .an('object');
// 				const token = res.body.authToken;
// 				expect(token).to.be.a('string');
...
// 					algorithm: ['HS256']
// 				});
// 				expect(payload.user).to.beep.equal({
// 					username,
// 					firstName,
// 					lastName
// 				});
